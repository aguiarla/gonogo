...
for k = sessions
    ...
    goInd = D(:,typeCol)==1;            % go trials are type 1
    nGo = histc(D(goInd,1),rng);
    nGo = nGo/length(D(goInd,1))/dt;
    
    noGoInd = D(:,typeCol)==2;          % go trials are type 2
    nNoGo = histc(D(noGoInd,1),rng);
    nNoGo = nNoGo/length(D(noGoInd,1))/dt;
    
    %    invInd = D(:,typeCol)==0;          % invalid trials
    %    nInv = histc(D(invInd,1),rng);
    %    nInv = nInv/length(D(invInd,1))/dt;
    ...
    lgnd{count} = ['go-sess.' num2str(k,'%g')]; %#ok<*AGROW>
    ...
    % --- checking statistics on distributions ---
    [h,p] = kstest2(D(goInd,1),D(noGoInd,1),0.05,'smaller');
    fprintf('\n\nKS-test');
    if h
        fprintf('\n\nThe Go distribution is smaller than the nogo\n');
    else
        fprintf('\n\nThere are no differences in Go and NoGo distributions\n');
    end
    fprintf('p=%f\n\n',p);
    % ---
    
    [h,p] = ttest2(D(goInd,1),D(noGoInd,1));
    fprintf('\n\nT-test');
    if h
        fprintf('\n\nThe Go distribution is smaller than the nogo\n');
    else
